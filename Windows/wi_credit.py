...
class WindowInputCredit(Window):
    ...
    def vis(self):
        ...
        # data
        client_id_block = create_option_menu(self.window, "ID klienta", "Wybierz klienta", get_select(self.db_name, "SELECT klient_id FROM klient;"), 10)
        credit_type_block = create_option_menu(self.window, "Typ kredytu", "Wybierz typ kredytu", get_select(self.db_name, "SELECT typ_kredytu_id FROM typ_kredytu;"), 10)
